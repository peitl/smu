/* genbg-almost.c : version 0.2; B D McKay, 16 May 2020.
   Modification to put a matching 0-1, 2-3, ... on the first side */

/* TODO: consider colour swaps */

#define USAGE \
"genbg-almost [-c -ugs -vq -lzF] [-Z#] [-D#] [-A] [-d#|-d#:#] [-D#|-D#:#] n1 n2 \n\
                [mine[:maxe]] [res/mod] [file]"

#define HELPTEXT \
" Find all almost-bicoloured graphs of a specified class.\n\
\n\
...
#define OUTPROC mufilter
// look at the mufilter procedure for further details
...
inline int h(uint32_t lit) {
	/* converts a literal lit represented as 2*(var(lit)-1) + neg?(lit)
	 * into (-1)^neg?(lit) * var(lit) */
	int var = lit / 2 + 1;
	if (lit % 2 == 0)
		return var;
	else
...
void print_clause(FILE* f, uint32_t * cl, uint32_t size) {
...
/* make extra space in clauses for selector vars
 * in the subsequent call to cryptominisat */
// actually, let's just make sure there's enough space overall
uint32_t clauses[MAXN][MAXN+1];
uint32_t cls_size[MAXN];

/* we need a selector variable for every clause, and, additionally,
 * for every literal, for the saturation check. See below */

nauty_counter total = 0;
nauty_counter num_sat = 0;
nauty_counter num_non_min = 0;
nauty_counter num_mu = 0;
nauty_counter num_saturated = 0;

int shiftwidth;
setword all_cls = 0;
setword cls_up_to[WORDSIZE];
setword non_ass;
setword max_var_val[WORDSIZE];
setword min_var_val[WORDSIZE];

clock_t mufilter_time = 0;

void init_filter_variables(int n1, int n2) {
	...
	non_ass = 1 << num_vars;
	int n = n1 + n2;
	shiftwidth = WORDSIZE - num_vars;

	cls_up_to[0] = 0;
	for (int i = 0; i < n2; i++) {
		cls_up_to[i+1] = cls_up_to[i] | bit[i+n1];
	}
	all_cls = cls_up_to[n2];

	sprintf(outdir, "smu_%u_%u", num_vars, num_cls);
	//sprintf(outdir, "tmp");

}

extern void writeg6x(FILE*, graph*, int, int);

void print_setword(FILE* f, setword s, int n) {
	int b, c = 0;
	while (s) {
		TAKEBIT(b, s);
		for (; c < b; c++)
...
void adjprint(FILE* f, graph* g, int n1, int n2) {
	int n = n1 + n2;
	for (int i = 0; i < n; i++) {
		print_setword(f, g[i], n);
	}
	fprintf(f, "---\n");
}

boolean is_graph_sat(graph* g, int n1, int n2) {
	// using global num_vars
	for (setword tau = 0; tau < non_ass; ++tau) {
		setword hit_cls = 0;
		for (int var = 0; var < num_vars; var++) {
			if (tau & bit[var + WORDSIZE - num_vars])
				hit_cls |= g[2*var];
			else
				hit_cls |= g[2*var+1];
		}
		if ((hit_cls & BITMASK(n1-1)) == all_cls)
			return TRUE;
	}
	return FALSE;
}

int count_graph_sat(graph* g, int n1, int n2) {
	// using global num_vars
	int num_models = 0;
	for (setword tau = 0; tau < non_ass; ++tau) {
		setword hit_cls = 0;
		...
		if ((hit_cls & BITMASK(n1-1)) == cls_up_to[n2])
			++num_models;
	}
	return num_models;
}

int mufilter_native(graph* g, int n1, int n2) {
	setword singly_falsified = 0;
	// for each clause i, max_var_val[i] gives for
	// each variable its maximum value over assignments
	// that uniquely falsify clause i
	// similarly for min_var_val
	for (int i = 0; i < n2; i++) {
		min_var_val[i] = -1;
		max_var_val[i] = 0;
	}
	for (setword tau = 0; tau < non_ass; ++tau) {
		setword hit_cls = 0;
		for (int var = 0; var < num_vars; var++) {
			if (tau & bit[var + shiftwidth])
				hit_cls |= g[2*var];
			else
				hit_cls |= g[2*var+1];
		}
		setword zeroset = (hit_cls & BITMASK(n1-1)) ^ all_cls; 
		if (zeroset == 0) {
			// satisfiable
			return 1;
		} else {
			...
			TAKEBIT(i, zeroset);
			if (zeroset == 0) {
				// only clause  i-n1  was falsified
				singly_falsified |= bit[i];
				i -= n1;
				min_var_val[i] &= tau;
				max_var_val[i] |= tau;
			}
	...
	if (singly_falsified != all_cls) {
		// not MU
		return 2;
	}
	for (int i = 0; i < n2; i++) {
		// exp has 1-bits for variables whose one literal can extend clause i
		// but we also need to exclude variables that are already in C!!
		// for now we solve this in a loop
		setword exp = (~(min_var_val[i] ^ max_var_val[i])) & BITMASK(shiftwidth-1);
		while (exp) {
			// clause i is not saturated with a literal of the variable
			// whose bit makes the above expression non-zero
			int l;
			TAKEBIT(l, exp);
			l -= shiftwidth;
			if (g[n1 + i] & bit[2*l] || g[n1 + i] & bit[2*l+1]) {
				// a literal on l is in clause i
				continue;
			}
			//fprintf(stderr, "Ext cl %d with lit %d\n", i, l);
			return 3;
		}
	}
...
void mufilter(FILE *f, graph *g, int n1, int n2) {
	clock_t mufilter_start = clock();
	total++;

	int r = mufilter_native(g, n1, n2);
	//int r = mufilter_mixed(g, n1, n2);
	...
	if (r == 0) {
		num_saturated++;
		//sprintf(musname, "%s/mus_%u.cnf", outdir, num_saturated);
		//FILE* mus = fopen(musname, "w");
		//fprintf(mus, "p cnf %u %u\n", num_vars, num_cls);
		//for (uint32_t cl_idx = 0; cl_idx < num_cls; cl_idx++) {
		//	// print clause without selector var
		//	print_clause(mus, clauses[cl_idx], cls_size[cl_idx] - 1);
		//}
		//fclose(mus);
		writeg6x(f, g, n1, n2);
	}

	mufilter_time += (clock() - mufilter_start);
}

#define SUMMARY mufilter_summary
#ifdef SUMMARY
extern void SUMMARY(nauty_counter,double);
#endif
	
void mufilter_summary(nauty_counter nout, double time) {

	nauty_counter num_unsat = total - num_sat;
	
	//fprintf(stderr, "-------------------------------\n");
	fprintf(stderr, ">Z SAT / total        : %lu/%lu (%.1f%%)\n", num_sat, total, 100.0 * num_sat / total);
	fprintf(stderr, ">Z Non-min / UNSAT    : %lu/%lu (%.1f%%)\n", num_non_min, num_unsat, 100.0 * num_non_min / num_unsat);
	fprintf(stderr, ">Z MUs / total        : %lu/%lu (%.1f%%)\n", num_mu, total, 100.0 * num_mu / total);
	fprintf(stderr, ">Z Saturated / MUs    : %lu/%lu (%.1f%%)\n", num_saturated, num_mu, 100.0 * num_saturated / num_mu);
	//fprintf(stderr, "-------------------------------\n");
	fprintf(stderr, ">Z Time for MU filter : %.3f sec\n", (double)(mufilter_time) / CLOCKS_PER_SEC);
}

#define PRUNE1 increasing_degs_be_enough_wa

int increasing_degs_be_enough_wa(graph *g, int *deg, int n1, int n2, int maxn2) {
	// this version assumes the first partition are literals
	// *AND* clause sizes are *nondecreasing*
	// num_vars is a global variable equal to n1/2

	// we first count the number of satisfying assignments, and then
	// check that our clauses are narrow enough to kill them all
	// (also, the formula must not be unsatisfiable unless n2 == maxn2)
	int n = n1 + n2;
	int num_sat_assignments;
	int tba = maxn2 - n2;

	if (n2 == 1) {
		num_sat_assignments = 1 << (num_vars - deg[n1]);
	} else if (tba == 0) {
		num_sat_assignments = is_graph_sat(g, n1, n2);
	} else {
		num_sat_assignments = count_graph_sat(g, n1, n2);
		if (num_sat_assignments == 0) {
			// an unsatisfiable subformula
			return -1;
		}
	}

	int future_cover_bound = tba * (1 << (num_vars - deg[n-1]));
	if (future_cover_bound < num_sat_assignments) {
		/* too many satisfying assignments
		 * so that even if all the rest have the smallest possible size,
		 * i.e. deg[n-1] (degs are increasing), the formula cannot
...
int increasing_degs_be_enough(graph *g, int *deg, int n1, int n2, int maxn2) {
	// this version assumes the first partition are literals
	// *AND* clause sizes are *nondecreasing*
	// num_vars is a global variable equal to n1/2
	int n = n1 + n2;
	...
	int i;
	for (i = n1; i < n; i++) {
		cover_sum += 1 << (num_vars - deg[i]);
	}
	cover_sum += (maxn2 - n2) * (1 << (num_vars - deg[n-1]));
	if (cover_sum < 1 << num_vars) {
		/* present clauses don't cover enough of the assignment space
		 * so that even if all the rest have the smallest possible size,
		 * i.e. deg[n-1] (degs are increasing), the formula cannot
		 * become unsatisfiable */
		return -1;
...
int degs_be_enough(graph *g, int *deg, int n1, int n2, int maxn2) {
	// this version assumes the first partition are literals
	// num_vars is a global variable equal to n1/2
	int limexp = num_vars + n2 - maxn2;
	if (limexp > 0) {
		int lim = 1 << limexp;
		int cover_sum = 0;
		...
		if (cover_sum < lim) {
			/* present clauses don't cover enough of the assignment space
			 * so that even if all the rest are unit clauses (on different
			 * variables, because otherwise it's not MU unless n1=n2=2),
			 * the formula cannot become unsatisfiable */
			// TODO: now we can actually assume that all clauses have cardinality >= 2
			return -1;
		}
	}
	return 0;
...
boolean trianglefree;           /* presence of -t  ALMOST */
...
static int oddbits; /* xbits 1,3,... up to n1  ALMOST */
...
abcdefghijklmnopqrstuvwxyz!\"#$%&'()*-/:;<=>?@[\\]^_`{|}~";
    ...
    {
        fprintf(stderr,">E genbg-almost : error on writing file\n");
        gt_abort(NULL);
    }
}
...
/* make the level data for each level */
{
    ...
    {
        fprintf(stderr,">E genbg-almost: malloc failed in makeleveldata()\n");
        exit(2);
    }

 /* ALMOST adjustments to this loop */
    nxsets = 0;
    for (i = 0; i < tttn; ++i)
    {
        if (trianglefree && (((i&oddbits) >> 1) & i)) continue;
    ...
    if (j != nxsets)
    {
        fprintf(stderr,">E genbg-almost: j=%d mxsets=%d\n",j,nxsets);
        exit(2);
    }
#endif
...
main(int argc, char *argv[])
{
    ...
    {
        fprintf(stderr,"genbg-almost: incompatible MAXN, MAXN1 or WORDSIZE\n");
    ...
    for (j = 1; !badargs && j < argc; ++j)
    {
        ...
        if (arg[0] == '-' && arg[1] != '\0')
        {
            ...
            while (*arg != '\0')
            {
                ...
                else SWINT('Y',gotY,mincommon,"genbg-almost -Y")
                else SWINT('Z',gotZ,maxcommon,"genbg-almost -Z")
                else SWINT('X',gotX,splitlevinc,"geng-almost -X")
                else SWRANGE('D',":-",gotD,Dval1,Dval2,"genbg-almost -D")
                else SWRANGE('d',":-",gotd,dval1,dval2,"genbg-almost -d")
...
PLUGIN_SWITCHES
        ...
        else
        {
            ...
            else
            {
                ...
                if (!gote)
                {
                    ...
                    {
                        gote = TRUE;
                        if (maxe == 0 && mine > 0) maxe = MAXN*MAXN/4 + MAXN/2; /* ALMOST */
                        continue;
                    }
    ...
    {
        fprintf(stderr,
           ">E genbg-almost: must have n1=1..%d, n1+n2=1..%d\n",MAXN1,MAXN);
        badargs = TRUE;
    }
 ...
 /* Many ALMOST changes in the following adjustments */
    if (maxdeg1 > maxn2+1) maxdeg1 = maxn2+1; 
    if (maxdeg2 > n1) maxdeg2 = n1;
    if (trianglefree && maxdeg2 > n1/2) maxdeg2 = n1/2;
    if (connec && mine < n1+maxn2-1) mine = n1 + maxn2 - 1;
    if (mindeg1 == 0) mindeg1 = 1; 
    if (connec && mindeg2 == 0) mindeg2 = 1;
    if (maxe > n1*maxdeg1 + n1/2) maxe =  n1*maxdeg1 + n1/2;
    if (maxe > maxn2*maxdeg2 + n1/2) maxe =  maxn2*maxdeg2 + n1/2;
    if (mine < n1*mindeg1 - n1/2) mine = n1*mindeg1 - n1/2;
    if (mine < maxn2*mindeg2 + n1/2) mine = maxn2*mindeg2 + n1/2;

    if (!badargs && (mine > maxe || maxe < 0 || maxdeg1 < 1 || maxdeg2 < 0))
    {
        fprintf(stderr,">E genbg-almost: impossible mine,maxe,maxdeg values\n");
        badargs = TRUE;
    }
    ...
    if ((nautyformat!=0) + (graph6!=0) + (greout!=0)
        ...
        gt_abort(">E genbg-almost: -ungsa are incompatible\n");
...
PLUGIN_INIT
    ...
    {
        fprintf(stderr,
              ">E genbg-almost: can't open %s for writing\n",outfilename);
        gt_abort(NULL);
    }
    ...
    if (!quiet)
    {
        ...
        if (trianglefree) CATMSG0("t");  /* ALMOST */
        if (connec) CATMSG0("c");
 ...
 /* ALMOST Initialise to a matching on the left side */
    if (n1%2 != 0) gt_abort(">E For this version n1 must be even\n");
 ...
 /* End ALMOST */
	...
	init_filter_variables(n1, maxn2);
    ...
    else
    {
        ...
        xbnds(0,n1/2,0);  /* ALMOST */
        imin = xstart[data[0].xlb];
        imax = xstart[data[0].xub+1];

   /* ALMOST these lines give the inequivalent nbrs of
        the first vertex in the second part */

        for (i = imin; i < imax; ++i)
            data[0].xorb[i] = -1;

	for (i2 = 0; i2 <= (trianglefree ? 0 : n1/2); ++i2)
	for (i1 = 0; i1 <= (n1-i2)/2; ++i1)
	{
	    msk = 0;
	    for (j2 = 0; j2 < i2; ++j2)
		msk |= xbit[2*j2] | xbit[2*j2+1];
	    for (j1 = 0; j1 < i1; ++j1)
		msk |= xbit[2*i2+2*j1];
        
	    if (XPOPCOUNT(msk) >= data[0].xlb &&
		       XPOPCOUNT(msk) <= data[0].xub)
	        data[0].xorb[xinv[msk]] = xinv[msk];
	}
   /* End ALMOST modifications */
#if 0
        for (i = data[0].xlb; i <= data[0].xub; ++i)
            data[0].xorb[xstart[i]] = xstart[i];
#endif

        genextend(g,0,deg,n1/2,FALSE,data[0].xlb,data[0].xub);
    }
    t2 = CPUTIME;
